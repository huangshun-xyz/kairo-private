...
- `time_types.h` 统一时间单位
- `frame_source.h` 提供动画帧推进
- `invalidation_sink.h` 负责请求宿主重绘

这背后的关键思想是：

- 时间单位不要散落成裸 `double`
- 逻辑层不直接依赖平台帧 API
- 初期先保留真正被 runtime 用到的边界

### 2. TaskRunner / Timer / TickClock 暂不引入

这些概念在更复杂的异步调度或测试场景里可能有价值，但当前仓库还没有真实消费点。

现阶段不必为了“未来可能会用”提前保留抽象。
